...
function Payment() {
  ...
  useEffect(() => {
    ...
    const getClientSecret = async () => {
      ...
      //console.log("Response is ", response); Mrrr
  ...
  const handleSubmit = async (event) => {
    ...
    const payload = await stripe
      ...
      // useEffect(() => {
      //   //generat the special stripe secret which is allows us to charge a customer
      //   const getClientSecret = async () => {
      //       const response = await axios.post( http://localhost:3030/payments/create?total=${getBasketTotal(basket) * 100}
      //       )
